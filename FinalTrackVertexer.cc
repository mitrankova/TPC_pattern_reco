...
FinalTrackVertexer::FinalTrackVertexer(const std::string& name)
  : SubsysReco(name)
  , m_inputNodeName("FINALTRACKS")
  , m_outputNodeName("FINALTRACKVERTICES")
{
}

int FinalTrackVertexer::InitRun(PHCompositeNode* topNode)
{
  if (!createNodes(topNode)) return Fun4AllReturnCodes::ABORTRUN;
  return Fun4AllReturnCodes::EVENT_OK;
}

bool FinalTrackVertexer::getNodes(PHCompositeNode* topNode)
{
  m_finalTracks = findNode::getClass<FinalTrackContainer>(topNode, m_inputNodeName.c_str());
  if (!m_finalTracks)
  {
    ...
    for (unsigned int i = 0;
         i < sizeof(candidate_names) / sizeof(candidate_names[0]) && !m_finalTracks;
         ++i)
    {
      m_finalTracks = findNode::getClass<FinalTrackContainer>(topNode, candidate_names[i]);
      if (m_finalTracks) m_inputNodeName = candidate_names[i];
    }
  }

  if (!m_finalTracks)
  {
    std::cerr << Name() << "::getNodes - missing FinalTrackContainer node" << std::endl;
    return false;
  }
...
bool FinalTrackVertexer::createNodes(PHCompositeNode* topNode)
{
  ...
  m_vertices = findNode::getClass<FinalTrackVertexContainer>(topNode, m_outputNodeName.c_str());
  if (!m_vertices)
  {
    m_vertices = new FinalTrackVertexContainerv1();
...
FinalTrackVertexer::TrackVertexFit
FinalTrackVertexer::fitTrack(const FinalTrack* trk) const
{
  TrackVertexFit fit;
  ...
  fit.source_full_track_id = trk->get_source_full_track_id();
  fit.nclusters = trk->get_nclusters();
  fit.ok = good(fit.dca2d) && good(fit.z0) && good(fit.pca_radius) && good(fit.pca_phi);
  return fit;
}

FinalTrackVertexer::CollisionFit
FinalTrackVertexer::fitCollision(const std::vector<TrackVertexFit>& tracks) const
{
  CollisionFit fit;
...
std::vector<FinalTrackVertexer::CollisionFit>
FinalTrackVertexer::fitCollisions(std::vector<TrackVertexFit> tracks) const
{
...
int FinalTrackVertexer::process_event(PHCompositeNode* topNode)
{
  if (!m_finalTracks || !m_vertices)
  {
  ...
  const unsigned int ntracks = m_finalTracks ? m_finalTracks->size() : 0;
  for (unsigned int itrk = 0; itrk < ntracks; ++itrk)
  {
    const FinalTrack* trk = m_finalTracks->get_track(itrk);
    const TrackVertexFit fit = fitTrack(trk);
    if (!fit.ok) continue;

    FinalTrackVertexv1* out = new FinalTrackVertexv1();
    out->set_track_id(fit.track_id);
    out->set_source_full_track_id(fit.source_full_track_id);
  ...
  if (Verbosity() > 0)
  {
    std::cout << Name() << "::process_event - input final_tracks=" << ntracks
