...
int IdealPadMap::load_from_cdb(const int verbosity)
{
  ...
  for (unsigned int ilayer = 0; ilayer < N_LAYERS; ++ilayer)
  {
    ...
    // Sorting makes local_phibin stable: 0..pads_per_sector-1 corresponds to
    // the ordered local CDB phi positions for this layer.
...
unsigned int IdealPadMap::get_pads_per_sector(const unsigned int region) const
{
  ...
  // Use the first layer of the region.  This avoids hard-coding 94/128/192;
  // the values come directly from the CDB map size.
                            ...
                            const unsigned int local_phibin) const
{
  ...
  const double cdb_phi = get_cdb_local_phi(layer, local_phibin);
  if (!std::isfinite(cdb_phi)) return std::numeric_limits<double>::quiet_NaN();

  // This is exactly the convention you quoted:
  // phi = ((side == 1 ? 1 : -1) * (CDB_phi - pi/2)) + sector*pi/6
